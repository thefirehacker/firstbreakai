...
[● Part A published]{.lesson-hero-status .lesson-hero-status-published}
:::
...
::: {.lesson-hero-meta}
[~37 min video · Part A]{.lesson-hero-chip} [3 parts total]{.lesson-hero-chip} [Cohort 01](../office-hours/2026-05-08.qmd){.lesson-hero-chip} [Run a model locally](../roadmap.qmd){.lesson-hero-chip}
:::
:::

This lesson ships in **three video parts**. **Part A** is live — run Qwen3 locally in pure C on your laptop or desktop. Parts B and C are on the way. The full written lesson below covers the file and memory layer in depth; the video mirrors it part by part. Click any transcript line to jump the video to that moment.
...
```{=html}
<div class="lesson-parts" role="tablist" aria-label="Lesson 1b video parts">
  <button type="button" class="lesson-part-tab active" role="tab" aria-selected="true" data-part="a" id="lessonPartTabA">
    <span class="lesson-part-label">Part A</span>
    <span class="lesson-part-title">Run Qwen3 locally</span>
    <span class="lesson-part-badge lesson-part-badge-live">Live</span>
  </button>
  <button type="button" class="lesson-part-tab" role="tab" aria-selected="false" data-part="b" id="lessonPartTabB">
    <span class="lesson-part-label">Part B</span>
    <span class="lesson-part-title">GGUF &amp; memory mapping</span>
    <span class="lesson-part-badge lesson-part-badge-soon">Coming soon</span>
  </button>
  <button type="button" class="lesson-part-tab" role="tab" aria-selected="false" data-part="c" id="lessonPartTabC">
    <span class="lesson-part-label">Part C</span>
    <span class="lesson-part-title">Conversion &amp; forward pass</span>
    <span class="lesson-part-badge lesson-part-badge-soon">Coming soon</span>
  </button>
</div>

<div class="lesson-part-panel active" data-part="a" id="lessonPartPanelA" role="tabpanel" aria-labelledby="lessonPartTabA">
<div class="lesson-page lesson-page-theatre" data-size="wide">

  <div class="lesson-deck">
    <div class="lesson-video-wrap">
      <div id="ytPlayer"
           data-video-id="YL-r5PMMtow"
           data-transcript="lesson-1b-qwen3-fundamentals-part-a.json"></div>
    </div>

    <div class="lesson-meta">
      <div class="lesson-now-playing">
        <span class="lesson-now-label">Chapter</span>
        <span class="lesson-chapter-title" id="lessonChapterTitle">Introduction: Run Qwen3 Locally</span>
      </div>
    ...
    <div class="lesson-chapters">
      <div class="lesson-chapters-label">Chapters · Part A</div>
...
<div class="lesson-part-panel" data-part="b" id="lessonPartPanelB" role="tabpanel" aria-labelledby="lessonPartTabB" hidden>
  <div class="lesson-part-soon-card">
    <div class="lesson-part-soon-icon" aria-hidden="true">B</div>
    <h3 class="lesson-part-soon-heading">Part B — Coming soon</h3>
    <p class="lesson-part-soon-desc">How a GGUF file is structured, memory mapping, and the pointer walk that wires weights into the forward pass. The written chapters below cover this material today — the video is being produced.</p>
    <p class="lesson-part-soon-note">Part A is live above. Check back after the next cohort drop, or read ahead in the written lesson.</p>
  </div>
</div>

<div class="lesson-part-panel" data-part="c" id="lessonPartPanelC" role="tabpanel" aria-labelledby="lessonPartTabC" hidden>
  <div class="lesson-part-soon-card">
    <div class="lesson-part-soon-icon" aria-hidden="true">C</div>
    <h3 class="lesson-part-soon-heading">Part C — Coming soon</h3>
    <p class="lesson-part-soon-desc">SafeTensors vs GGUF, the HuggingFace-to-GGUF conversion pipeline, and how logits become tokens — prefill, sampling, and the KV cache.</p>
    <p class="lesson-part-soon-note">Complete Part A first, then work through the written lesson while you wait for Parts B and C.</p>
  </div>
</div>

<script>
(function() {
  var tabs = document.querySelectorAll('.lesson-part-tab');
  var panels = document.querySelectorAll('.lesson-part-panel');
  tabs.forEach(function(tab) {
    tab.addEventListener('click', function() {
      ...
      panels.forEach(function(p) {
        var show = p.getAttribute('data-part') === part;
...
## Chapter 8: Inside the Transformer Layer — Attention and the MLP

Chapter 7 showed you the forward pass as code — ten steps per layer. But *why* does each layer have this structure? Each of the 28 layers in Qwen3-0.6B has two main blocks:

```
Transformer Layer
 ├── Attention block       ← tokens talk to each other
 │    ├── q_proj (wq)
 │    ├── k_proj (wk)
 │    ├── v_proj (wv)
 │    └── o_proj (wo)
 │
 └── MLP / FFN block       ← each token refines its own representation
      ├── gate_proj (w3)
      ├── up_proj (w1)
      └── down_proj (w2)
```

Attention mixes information *across* tokens ("which other tokens should I look at?"). The MLP transforms each token's representation *independently* ("after seeing the context, how should I update this token's meaning?"). Both are required — one without the other is insufficient.

### Multi-Head Attention — Why 16 Smaller Views Beat One Big One

#### Single-head attention (the starting point)

In the simplest version, each token gets one query, one key, and one value — all of size `dim` (1024). The attention score is a single dot product between Q and K, and the output is a single weighted sum of V. This gives each token *one way* of looking at the sequence.

#### The problem with one head

A token often needs to attend to different things simultaneously. Consider:

```
"The animal didn't cross the road because it was tired."
```

For the token "it", the model needs to:

- Connect "it" to "animal" (coreference)
- Track negation from "didn't" (semantics)
- Understand "tired" as a state (predicate)
- Recognize phrase boundaries (syntax)

One attention pattern can't do all of this at once. It produces one set of attention weights — one "view" of the sequence.

#### Multi-head: many smaller views in parallel

Instead of one big attention with 1024-dimensional Q, K, V, the model splits them into **16 heads of 128 dimensions each**:

```
hidden_size = 1024
n_heads     = 16
head_dim    = 1024 / 16 = 64... wait — Qwen3 uses head_dim = 128
```

Qwen3's head_dim is 128, not 1024/16 = 64. So the full Q projection output is actually `n_heads × head_dim = 16 × 128 = 2048`, which is *larger* than the hidden size. This is why `wq` has shape `[2048, 1024]` — it projects 1024-dim input into 2048-dim Q space.

#### Step by step in `run.c`

**Step 1: Create Q, K, V.** The projections produce the full output:

```c
matmul(s->q, s->xb, w->wq + l * layer_offset, p->dim, att_head_dim);
// s->q shape: [2048] = 16 heads × 128 dim per head

matmul(s->k, s->xb, w->wk + l * layer_offset, p->dim, kv_dim);
// s->k shape: [1024] = 8 KV heads × 128 dim per head (GQA!)
```

**Step 2: Split into heads.** The code doesn't explicitly reshape — it uses pointer arithmetic. Each head's Q is a slice of 128 floats:

```c
for (int h = 0; h < p->n_heads; h++) {
    float* q = s->q + h * p->head_dim;  // head 0: q[0..127], head 1: q[128..255], ...
```

This is the "reshape" that frameworks like PyTorch do with `view()` and `transpose()` — but in C, it's just a pointer offset.

**Step 3: Each head does independent attention.** Every head computes its own attention scores (dot products of Q with all past K vectors), applies softmax, and produces a weighted sum of V:

```c
    float* att = s->att + h * p->seq_len;  // this head's score buffer
...
Each head produces 128 floats. 16 heads × 128 = 2048 floats total, sitting contiguously in `s->xb3`.

**Step 4: Output projection.** The `o_proj` (`w->wo`) mixes information across heads and projects back to 1024 dimensions:
...
Without `o_proj`, the head outputs would remain separated — head 0's 128 floats wouldn't interact with head 7's 128 floats. The output projection lets the model combine what different heads learned.
...
Input x [1024]
     │
     ├─→ wq ─→ Q [2048] ─→ split into 16 heads of [128]
     ├─→ wk ─→ K [1024] ─→ split into  8 heads of [128] (GQA)
     └─→ wv ─→ V [1024] ─→ split into  8 heads of [128] (GQA)
                                │
                    ┌───────────┼───────────┐
...
Each head can learn a different relationship. Head 3 might track coreference while head 11 tracks syntactic structure. The model doesn't assign these roles explicitly — the heads discover useful patterns during training.

### The MLP Block — Up, Gate, and Down Projections

After attention, each token has gathered context from the sequence. Now the MLP transforms each token's representation *independently* — no cross-token communication.

#### Why not just 1024 → 1024?

A direct projection from 1024 to 1024 is limited. The MLP expands to a larger internal space, works in that space, and compresses back:

```
1024  →  3072  →  1024
```

This is like solving a problem by expanding your working space, doing the reasoning, then compressing the final result. The larger intermediate dimension gives the network more expressive capacity to represent complex patterns.

#### The three projections in Qwen3

Qwen3 uses a **gated MLP** (SwiGLU style) with three projections:

| Projection | `run.c` name | Shape | Role |
|-----------|-------------|-------|------|
| `up_proj` | `w->w1` | [1024, 3072] | Expand to candidate features |
| `gate_proj` | `w->w3` | [1024, 3072] | Control which features pass |
| `down_proj` | `w->w2` | [3072, 1024] | Compress back to hidden size |

The computation:

```
x ──→ gate_proj ──→ SiLU activation ──┐
                                       × ──→ down_proj ──→ output
x ──→ up_proj ────────────────────────┘
```

Or as a formula: `output = down_proj( SiLU(gate_proj(x)) × up_proj(x) )`

#### The code in `run.c`

```c
// up_proj: x [1024] → hb [3072]
matmul(s->hb, s->xb, w->w1 + l * layer_offset, p->dim, p->hidden_dim);

// gate_proj: x [1024] → hb2 [3072]
...
for (int i = 0; i < p->hidden_dim; i++) {
    ...
    val *= s->hb[i];                       // elementwise multiply with up_proj
    s->hb2[i] = val;
}
...
The `gate_proj` acts as a learned filter. It decides which of the 3072 intermediate features matter for this token. `up_proj` creates the candidate features. The elementwise multiply combines them — features where the gate is near zero get suppressed, features where the gate is large pass through.

#### Intuition: the "bank" analogy

Suppose a token's 1024-dim vector encodes the compressed meaning of `"bank"`. Depending on context, it could mean a river bank, a money bank, a banking API, or a bank angle in aviation. The MLP expands this into 3072 dimensions — room to create intermediate features like "is this financial?", "is this geographical?", "is this code?" (not with human labels, but conceptually). The gate selects which features are relevant given the context that attention already gathered. Then `down_proj` compresses the refined representation back to 1024 for the next layer.

### Why Both Blocks Are Needed

| Block | What it does | Operates on |
|-------|-------------|-------------|
| **Attention** | Mixes information across tokens | All tokens in the sequence |
| **MLP** | Transforms features within each token | Each token independently |

Attention answers: *"Which other tokens should I look at?"*

The MLP answers: *"After seeing the context, how should I update this token's meaning?"*

A model with only attention could gather context but couldn't transform it. A model with only MLPs could transform tokens but each token would be isolated — unable to see anything else in the sequence. The alternating attention-MLP structure, repeated 28 times, builds increasingly sophisticated representations.

### The Complete Layer Flow

Putting it all together — one of the 28 identical layers:
...
Input x [1024]
     ...
     │     │
     │     ├─→ Q, K, V projections
     │     ├─→ QK norms + RoPE (position encoding)
...
The residual connections (`s->x[i] += ...`) are critical — they let the input pass through unchanged and only *add* what attention and the MLP learned. This makes deep networks trainable; without residuals, 28 layers of transformations would be nearly impossible to optimize.

---

## Chapter 9: How llama.cpp Does It Differently
...
## Chapter 10: The Custom Conversion Script — Making the Two Worlds Meet
...
## Chapter 11: Putting It All Together — The Full Pipeline
...
## Chapter 12: SafeTensors — The Other File Format
...
## Chapter 13: SafeTensors vs GGUF — A Detailed Comparison
...
## Chapter 14: Tensor Names — Framework Names vs Standardized Names
...
## Chapter 15: The Conversion Pipeline — SafeTensors In, GGUF Out
...
## Chapter 16: From Logits to Text — Prefill, Sampling, and What You Can See
...
### Why TTFT Feels Slow in Practice

A common reaction when running the model locally: *"My prompt is tiny — why does it take 30+ seconds before the first word appears?"*

The answer is **not** that the KV cache for your prompt is large. Two separate things get confused:

| Concept | What it means |
|---------|---------------|
| **KV cache used** | Grows with **prompt length** — a few MB for a short question |
| **KV cache allocated** | Reserved at startup for **max context** — huge regardless of prompt |

From `malloc_run_state()` in `run.c`, the allocation is:

```
2 × 28 layers × 40,960 positions × 1,024 kv_dim × 4 bytes ≈ 9.4 GB
```

That buffer is `calloc`'d at startup, even if your prompt is only 50 tokens. On a machine with 8 GB RAM already at ~87% usage, this triggers **swap to disk** and makes everything feel stuck.

#### What Actually Happens Before the First Token

```
You type a question + Enter
    → encode prompt to tokens (chat template, system prompt, etc.)
    → PREFILL: forward() once per prompt token (28 layers, CPU, FP32)
    → first generated token printed ← this is TTFT
```

**Prefill is heavy compute**: for each prompt token, the model runs a full forward pass with attention over all positions so far. TTFT is dominated by prefill + cold start, not by "loading" KV for your short prompt.

#### Factors That Stack Up

| Factor | Effect on TTFT |
|--------|----------------|
| **Low free RAM** | 9.4 GB KV alloc + 2.4 GB model mmap → swap to disk |
| **CPU-only FP32** | ~600M params, no GPU acceleration in `run.c` |
| **Cold mmap** | First touches of 2.4 GB GGUF load pages from disk |
| **Reasoning on** (`-k 1` default) | Long `<think>` block before the visible answer |
| **Viz on** (`-v 1`) | Copies 151,936 logits per step + bridge + dashboard |
| **System prompt + chat template** | Extra prefill tokens before generation starts |

#### Practical Tips (No OpenMP Required)

1. **Free RAM first.** Close browser tabs, other apps. Target at least 3–4 GB free before `./run`.
2. **Measure cleanly.** Turn off extras while benchmarking:
   ```bash
   ./run Qwen3-0.6B-FP32.gguf -f 1 -k 0 -v 0
   ```
   `-f 1` prints TTFT, `-k 0` skips the thinking block, `-v 0` disables viz overhead.
3. **Shorter prompts = fewer prefill steps.** Press Enter at "Enter system prompt" to skip it. Each extra token is one more full forward pass.
4. **Use viz for learning, not speed tests.** The dashboard is for understanding sampling. For TTFT comparison, use plain `./run` with `-f 1`.
5. **WSL users: move the repo off `/mnt/d/`.** Windows drives under `/mnt/` have slower I/O for heavy mmap. Copy to `~/` for better page fault performance.

#### What Will *Not* Help TTFT

| Change | Why |
|--------|-----|
| `-t 0` (greedy) | Affects **which** token is picked, not prefill speed |
| `-p 0.5` (smaller nucleus) | Same — sampling only, after prefill |
| Lower temperature | Dashboard experiments, not prefill |

Prefill cost is **CPU × prompt length × model size**. Sampling flags change generation behavior, not the initial prompt crunch.
...
### When an Unlikely Token Wins — Probability Is Chance, Not Destiny

Here is a real step from the sampling visualizer. The model was asked "who was the king in the jungle book" and at this decode step, with T=0.60 and top-p=0.95, the nucleus had 9 tokens:

![Sampling visualizer — "original" step. Nucleus size = 9 tokens covering 96.1% of probability mass. The top token " \\"" has 25.2%, " book" has 24.5%, " story" has 23.7% — but the model chose " original" at just 1.4% (gold bar, rank 9). 151,927 tail tokens excluded.](../public/images/lessons/sampling-viz-original-top.png){fig-alt="Qwen3 sampling visualizer showing 9-token nucleus where unlikely token original was chosen"}

![Same step, scrolled down. The generated text shows the model thinking through its reasoning: the chosen " original" token became part of the phrase 'the original Jungle Book'. The green bars show all 9 nucleus tokens; grey bars show tail tokens with negligible probability.](../public/images/lessons/sampling-viz-original-bottom.png){fig-alt="Qwen3 sampling visualizer showing generated tokens with original token selected in context"}

The natural question: *why did " original" win when " book" was ~17x more likely?*

**The model does not always pick the highest-probability token.** With default settings, it randomly samples from the nucleus. A 1.4% outcome is unlikely but valid — like drawing a rare ticket from a weighted lottery.

#### The Weighted Lottery

Imagine 100 tickets in a bag:
...
You pull **one** ticket. Usually you get `" book"` or `" story"`. Sometimes you get `" original"`. That is stochastic sampling, not a bug.

The relevant code in `sample_topp()`:
...
The `coin` is a uniform random number in [0, 1). If it lands in the 1.4% slice belonging to `" original"`, that token wins. Run the same prompt again with a different random seed and the model will likely pick `" book"` or `" story"` instead.

#### Why Not Always Pick the Top Token?

| Mode | Flag | Behavior |
|------|------|----------|
| **Greedy** | `-t 0` | Always `argmax` — highest probability every time |
| **Sampling** | `-t 0.6` (default) | Weighted random — more natural, varied text |
| **Top-p** | `-p 0.95` (default) | Ignore the long tail so output stays coherent |

Greedy text can be repetitive and mechanical. Sampling trades a bit of unpredictability for variety — the same reason human writers don't always pick the most obvious word. Top-p prevents sampling from the bottom 99.99% of unlikely tokens, so the output stays coherent even with randomness.

#### Experiments to Try

Run the bridge with different flags and watch how the dashboard changes:

```bash
# Sharper — top tokens win more often, fewer surprises
node ../../tools/sampling-bridge.mjs ./run_viz Qwen3-0.6B-FP32.gguf -v 1 -t 0.2 -p 0.95

# More surprises — flatter distribution, wider nucleus
node ../../tools/sampling-bridge.mjs ./run_viz Qwen3-0.6B-FP32.gguf -v 1 -t 1.2 -p 0.95

# Smaller nucleus — fewer candidates eligible
node ../../tools/sampling-bridge.mjs ./run_viz Qwen3-0.6B-FP32.gguf -v 1 -t 0.6 -p 0.5

# Always pick #1 (no lottery) — compare on the same prompt
./run Qwen3-0.6B-FP32.gguf -t 0
```
...
### What Top Labs Actually Use — Nucleus Sampling in Production

You've just learned nucleus sampling on a 0.6B model running on your laptop. Here's the thing: **this is the same sampling strategy that every major lab uses in production.** Temperature + top-p (nucleus) is the universal default across the industry.

#### The Industry Landscape

| Lab / Model | Temperature | Top-p | Notes |
|-------------|-------------|-------|-------|
| **OpenAI** (GPT-4, etc.) | user-configurable | user-configurable | "Set one, not both" — they interact in hard-to-predict ways |
| **OpenAI reasoning** (o1, o3, o4-mini) | **fixed at 1.0** | **fixed at 1.0** | API *rejects* any attempt to change them |
| **Anthropic** (Claude) | user-configurable | user-configurable | Extended-thinking mode only allows top_p in 0.95–1.0 |
| **Qwen3** (thinking mode) | 0.6 | 0.95 | "Do not use greedy decoding" — endless repetitions |
| **Qwen3** (non-thinking) | 0.7 | 0.8 | Also uses top_k=20 and presence penalty=1.5 |
| **DeepSeek-R1** | 0.6 | 0.95 | Benchmarks use 64 samples per prompt for pass@1 |
| **DeepSeek-V3** | 1.0 (API) | 0.95 | Hidden mapping: T_internal = T_API × 0.3, so "1.0" is actually 0.3 |
| **Kimi K2** (Moonshot AI) | 0.6 | — | API maps temperature: real_T = request_T × 0.6 |

Top-k has largely fallen out of favor at the API level because nucleus is adaptive — the cutoff expands when the model is uncertain, shrinks when it's confident — while top-k is a static count regardless of the distribution shape.

A newer technique worth knowing: **min-p sampling** (ICLR 2025). It uses a relative threshold — keep any token whose probability is at least `p × p_max` — which behaves better than top-p at high temperatures, where flat distributions let top-p admit garbage tokens. Qwen3 has started reporting min-p in its recommended configs.

#### Why Reasoning Models Need Stochastic Sampling

This is the most important insight from the table above: **reasoning models actively forbid greedy decoding.**

OpenAI's o1/o3 models fix temperature at 1.0 and reject API calls that try to change it. Qwen3's docs warn explicitly: *"Do not use greedy decoding, as it can lead to performance degradation and endless repetitions."*

The reason is the same for both: these models were trained with reinforcement learning to produce long self-correcting reasoning chains. The RL rollouts during training were generated with stochastic sampling, so the model *learned* to explore — "let me try X... wait, that doesn't work because... let me reconsider..." — and that exploration depends on occasionally picking a non-top token.

Collapsing to greedy (`-t 0`) kills this. The model always takes the most probable next token, which means it can never deviate from its first idea, never self-correct, never explore alternative reasoning paths. The chain degenerates into repetition loops.

You can observe this directly with your visualizer:

- **`-t 0.0` (greedy):** Watch the model get stuck repeating phrases. The gold bar is always on the top token — no exploration, no recovery from bad paths.
- **`-t 0.6` (recommended):** The top token still wins most of the time, but occasionally a second or third token wins. These are the moments where the model "changes its mind" mid-reasoning.
- **`-t 1.2` (too hot):** The distribution gets so flat that the model picks near-random tokens. Reasoning becomes incoherent.

The sweet spot at 0.6 means: *mostly confident, but with enough noise to self-correct.*

#### Why OpenAI Fixes at 1.0 but Qwen3 Recommends 0.6

This seems contradictory — both say "reasoning needs stochasticity," but OpenAI locks temperature to 1.0 while Qwen3 recommends 0.6. Three possible reasons:

1. **Hidden rescaling.** DeepSeek-V3 maps `T_API × 0.3` internally; Kimi K2 maps `T_API × 0.6`. OpenAI's "1.0" might actually be ~0.3–0.6 inside the model. We don't know because it's closed-source.
2. **Different RL training.** Qwen3's RL rollouts may have been generated at lower temperature, so the model learned to explore at 0.6 rather than 1.0.
3. **Quality tradeoff.** At T=1.0 the reasoning chain explores more but also gets noisier. Qwen3 found 0.6 to be the sweet spot between "enough exploration for self-correction" and "not so much noise that chains go off the rails."

The design philosophy also differs: OpenAI is paternalistic (prevents the footgun at the API level), while Qwen3 is open-source and trusts you to read the docs.

#### The Pattern Across the Industry

Once you line up all the labs, a consistent picture emerges:

- **Production APIs:** temperature + top-p (nucleus). Often "tune one, not both."
- **Reasoning/thinking modes:** temperature pushed up (0.6–1.0), top_p kept high (0.95–1.0), greedy actively discouraged.
- **Non-thinking instruct modes:** temperature lower (0.6–0.7), top_p tighter (0.8), often a presence penalty added to suppress repetition.
- **Benchmark reporting:** many samples per prompt (DeepSeek-R1 uses 64 completions per query), averaged for pass@k.
- **Pretraining ablations:** greedy for reproducibility — deterministic results across runs.

::: {.callout-tip}
## Recommended reading
The **DeepSeek-R1 model card** ([arXiv 2501.12948](https://arxiv.org/abs/2501.12948)) is the shortest, most explicit recipe — it shows the connection between nucleus sampling and pass@k evaluation in two paragraphs. The **Qwen3 technical report** ([arXiv 2505.09388](https://arxiv.org/abs/2505.09388)) publishes separate sampling configs for thinking vs non-thinking modes, directly relevant to what you're running locally.
:::
...
## Chapter 17: The KV Cache — Why the Model Remembers
...
Now the connection from Chapter 16 is complete:
