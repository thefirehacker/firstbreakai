...
::: {.lesson-hero-meta}
[~45 min read]{.lesson-hero-chip} [Cohort 01](../office-hours/2026-05-08.qmd){.lesson-hero-chip} [Run a model locally](../roadmap.qmd){.lesson-hero-chip}
:::
:::
...
The cohort has built a live dashboard that visualizes exactly what happens at every decode step. When you run the model with `-v 1`, each generated token emits a JSONL event containing the top-20 token probabilities, nucleus membership, and which token was chosen.
...
The visualizer makes several things concrete that are hard to see from numbers alone:

- **Nucleus size varies per step.** Sometimes only 2–3 tokens cover 95% of the mass; other times 8–10 do. The model's confidence changes with every token.
- **Temperature's effect is visible.** At 0.2, the top token bar fills the entire chart. At 1.2, the distribution flattens and the nucleus grows.
- **The tail is enormous.** Out of 151,936 tokens, typically only 3–10 are in the nucleus. The other 151,926+ are excluded — they had near-zero probability after temperature scaling.
