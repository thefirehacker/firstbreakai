...
```{mermaid}
flowchart TD
    A["You type a message"] --> B["Chat Template<br/>Wraps it in ChatML format"]
    B --> C["Tokenizer<br/>Breaks text into tokens"]
    C --> D["Token IDs<br/>Integers the model understands"]
    D --> E["Transformer<br/>28 layers of attention + FFN"]
    E --> F["Logits<br/>151936 scores, one per token"]
    F --> G["Sampler<br/>Temperature + top-p picks next token"]
    G --> H["Decoder<br/>Token ID back to text"]
...
```{mermaid}
flowchart LR
    P["Everything before:<br/>'The capital of France is'"] --> M["LLM<br/>(600M numbers)"]
    M --> N["Next most likely token:<br/>'Paris'"]
    N --> R["Repeat:<br/>'The capital of France is Paris'"]
    R --> M2["LLM"]
    M2 --> N2["'.'"]
```
...
```{mermaid}
flowchart TD
    SP["system_prompt<br/>e.g. 'You are a pirate'"]
    UP["user_prompt<br/>e.g. 'What is the weather?'"]
    SP --> TPL["sprintf into rendered_prompt<br/>using ChatML template"]
    UP --> TPL
    TPL --> RP["rendered_prompt:<br/>im_start system ... im_end<br/>im_start user ... im_end<br/>im_start assistant"]
...
```{mermaid}
flowchart TD
    TOK["Token ID<br/>e.g. 9906 = 'Hello'"] --> EMB["Embedding lookup<br/>row 9906 of the table<br/>→ 1024 floats"]
    EMB --> L1["Layer 1<br/>Attention + FFN"]
    L1 --> L2["Layer 2<br/>Attention + FFN"]
    L2 --> DOT["..."]
    DOT -->     L28["Layer 28<br/>Attention + FFN"]
    L28 --> RN["Final RMSNorm"]
    RN --> LG["matmul → logits<br/>151936 scores"]
```
...
flowchart TD
    ...
    RN1 --> ATT["Multi-head Attention<br/>(see Lesson 6)"]
    ATT --> ADD1["x = x + attention_output<br/>residual connection"]
    ADD1 --> RN2["RMSNorm"]
    RN2 --> FFN["Feed-forward Network<br/>SwiGLU activation"]
    FFN --> ADD2["x = x + ffn_output<br/>residual connection"]
    ADD2 --> XOUT["x (updated, 1024 floats)<br/>→ next layer"]
```
...
flowchart TD
    subgraph preAttn ["Before Attention"]
        x1["x dim=1024"] --> rn1["rms_att_weight<br/>RMSNorm"]
        rn1 --> proj["Wq, Wk, Wv projections"]
    end

    subgraph preFFN ["Before FFN"]
        x2["x dim=1024"] --> rn2["rms_ffn_weight<br/>RMSNorm"]
        rn2 --> ffn["SwiGLU FFN"]
    end
```
...
flowchart TD
    ...
    Q3 --> S1["Score = Q3·K1<br/>(how much 'it' attends to 'trophy')"]
    Q3 --> S2["Score = Q3·K2<br/>(how much 'it' attends to 'suitcase')"]
    S1 --> SF["softmax → attention weights"]
    S2 --> SF
    SF --> OUT["Weighted sum of Values<br/>= contextualised representation of 'it'"]
```
...
```{mermaid}
flowchart TD
    x["x 1024"] --> rmsA["RMSNorm<br/>rms_att_weight"]
    rmsA --> wq["Wq → Q"]
    rmsA --> wk["Wk → K"]
    rmsA --> wv["Wv → V"]
    wq --> qknQ["QK-Norm on Q<br/>wq_norm per head"]
    wk --> qknK["QK-Norm on K<br/>wk_norm per head"]
    qknQ --> rope["RoPE"]
    qknK --> rope
...
::: {.callout-important}
## Qwen3 0.6B has QK-Norm

Your GGUF has `attn_q_norm` and `attn_k_norm` tensors for all 28 layers, and `run.c` applies them. When you read HuggingFace configs, look for `q_norm` / `k_norm` under `self_attn` — they are present in Qwen3 dense models including 0.6B.
:::
...
flowchart TB
    ...
    subgraph attn ["Attention sub-block"]
        rmsA["RMSNorm 1024<br/>pre-attn"]
        wq["wq 1024→2048"]
        wk["wk 1024→1024"]
        wv["wv 1024→1024"]
        qkn["QK-Norm on Q,K<br/>128 per head"]
...
1. **GGUF tensors:** `blk.0.attn_q_norm.weight` and `blk.0.attn_k_norm.weight` (128 floats each)
...
QK-Norm attacks this **inside attention**. RMSNorm on `x` alone did not remove the problem at 32B — Marin tried three optimizer-level mitigations first. See [Reading the Curves — Marin case study](reading-the-curves-llm-training.qmd#case-study-marin-32b-the-full-timeline).
...
```{mermaid}
flowchart TD
    L["logits[151936]<br/>raw scores from transformer"] --> T["÷ temperature"]
    T --> SM["softmax → probabilities"]
    SM --> S1{sampling strategy}
    S1 -- "temperature=0, greedy" --> ARG["argmax: always pick<br/>highest probability"]
    S1 -- "top-p=0.95, default" --> TP["nucleus: keep<br/>smallest set ≥ 95% mass"]
    TP --> RND["sample randomly<br/>from filtered set"]
    S1 -- "temperature=1.5, random" --> FULL["sample from<br/>full distribution"]
...
flowchart LR
    ...
    subgraph virt ["Virtual Memory (mmap)"]
        PTR["float* pointers<br/>zero-copy"]
    end
...
flowchart TD
    subgraph startup ["Startup — runs once"]
        G["GGUF file (3 GB)"] -- "mmap()" --> TW["TransformerWeights<br/>(pointers, zero-copy)"]
        HT["header.txt"] --> CFG["Config<br/>dim=1024, layers=28..."]
    ...
    subgraph turn ["Each conversation turn"]
        ...
        SYS -->         TPL["ChatML Template<br/>im_start system...im_end<br/>im_start user...im_end<br/>im_start assistant"]
        TPL --> ENC["encode()<br/>BPE merge → token IDs"]
        ENC --> PF["Prefill phase<br/>forward() × prompt_length<br/>fills KV cache"]
        PF --> GEN["Generation phase<br/>forward() × 1 → sample → next token"]
        GEN --> DEC["decode_token_id()"]
        DEC --> PRINT["printf + fflush<br/>stream to screen"]
...
Join the [First Break AI Discord](https://discord.gg/hRPese4H3F) to share what you built and get help on the exercises.
