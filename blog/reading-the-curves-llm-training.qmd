...
- **General (S1)** — on the order of **tens of trillions** of tokens at **4,096** context, broad language and world knowledge.  
- **Reasoning-leaning (S2)** — on the order of **~5T** more tokens with **higher** STEM / code / reasoning / synthetic *share*, still at 4,096—LR decay is **accelerated** in that phase.  
- **Long context** — **hundreds of billions** of tokens at **32,768** context, with long-doc corpora; RoPE frequency and YARN/DCA-style techniques appear for length extension (see report).
...
### W&B and published plots

Qwen’s team publishes through **Hugging Face** ([Qwen org](https://huggingface.co/Qwen)), GitHub, and the **arXiv** report. This post does **not** embed their figures; open the PDF, search for “pre-training” / “loss,” and read the *caption* for **axes** and **stage** boundaries.
...
#### Data mix (Marin) — not Dolma, still “read the table”

Marin 32B pretrain uses a **Nemotron-CC–centric** mixture (starcoder + proofpile slices in small percentages). **That is not OLMo’s Dolma**—the lesson is: when you read *any* `loss`, you should know *which mixture* and *which batch schedule* (the report’s batch table changes with **hardware** and **divisibility**). Same skill you need for *your* W&B.
...
- **Spikes** are data; the Marin retrospective shows that **clipping** can be necessary but *insufficient* when the root is **attention** at scale.  
- **Public** artifacts (Marin, OLMo, Qwen3) exist—**cite them** when you make claims.

**Exercises (longer, deliberate)**

1. **OLMo** — Enumerate 10 `revision` tags for `allenai/OLMo-7B-0424-hf` (or the card’s recommended id), pick **two** far apart, run the same **three** prompts, and **tabulate** qualitative differences.  
2. **W&B** — For your next run, add **config** to the run page (YAML snapshot). When something “looks weird,” first compare **code SHA** and **data path** to the previous good run.  
3. **Marin** — Read [Phase 1–3](https://marin.readthedocs.io/en/latest/reports/marin-32b-retro/) once with the **Data Mix** and **Optimizer** pages open. Write a one-page **timeline** in your own words.  
4. **Qwen3** — In [arXiv:2505.09388](https://arxiv.org/abs/2505.09388), find **where** the three pretrain **stages** are listed and what **x-axis** their loss figures use.  
5. **Perfetto** — One trace from your own speedrun, **three** bullet **findings** (data vs comm vs kernel).
