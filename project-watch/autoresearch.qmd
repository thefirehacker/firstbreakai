...
- [Hands-on: Build your own autoresearch](#hands-on-build-your-own-autoresearch)
...
1. **Subprocess isolation** — `my_train.py` runs in a separate process. If it crashes, the evaluation catches it.
...
Propose a SINGLE concrete change to my_train.py that will reduce the MSE.
Return the COMPLETE new file content wrapped in ```python ... ``` markers.
Only change the coefficients list or add more terms. Do not change the
evaluation logic (test_points, the METRIC print line).
"""
    ...
    raise NotImplementedError(
        "Replace ask_agent() with your LLM API call. "
...
def run_loop(n_iterations=10):
    git("init")
    git("add my_train.py my_eval.py")
...
- [ ] Build your own autoresearch loop (`my_train.py`, `my_eval.py`, `my_loop.py`)
- [ ] Run 10 iterations and check `git log --oneline`

**Verification:**

- [ ] Can you explain Goodharting and how the fixed eval prevents it?
- [ ] Can you name the four failure modes of the naive loop?
- [ ] Did you experience those failures in your own loop?
